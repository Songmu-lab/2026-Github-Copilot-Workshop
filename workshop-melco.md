...
本日利用する `MitsubishiElectric-Innersource` オーガニゼーション配下にリポジトリを作成してください。リポジトリ名は `workshop-{あなたのユーザー名}` などとしてください（例: `workshop-johndoe`）。
...
ここから先のステップでは、GitHub.com上でのCopilot機能やCloud Agentを使用します。そのために必要な設定がされているか念のため確認しましょう。
...
プロンプトを書いたり、文脈を指定する際に、頻繁に同じような指示を出している場合は、Copilotにその指示を覚えさせることができます。具体的には、プロジェクト内に `AGENTS.md` や `.github/copilot-instructions.md` というファイルを作成し、その中に指示を書いておきます。このファイルがあると、Copilotはその指示を自動的に読み込み、以降のチャットでその指示を参照することができます。
...
## Pull requestのマージと動作確認

必要な修正を取り込んで、テストがすべてパスしたら、Pull Requestをマージしましょう。

![](https://gyazo.com/5b849a3a8ccf451a93fbab7fe9d1fc2d.png)

改めて、テストサーバーを起動して、ちゃんと動くか確認してみましょう。

## Copilot Cloud Agentによる追加機能開発 (オプション)
Duration: 20

Cloud Agentを活用して、さらに機能追加を行ってみましょう。

Cloud Agentの起動方法はいくつかありますが、ここでは、プロジェクトの改善提案をIssueとして自動生成し、Cloud Agentを活用してみましょう。

(IssueやPull Requestを事前に作らずとも、ブラウザ上のチャットから実装を進めてもらうことも出来ます。)
